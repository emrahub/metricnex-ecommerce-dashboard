  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      try {
        ...
        // Load metrics and reports in parallel
        const [metricsResponse, reportsResponse] = await Promise.all([
          dashboardService.getMetrics(),
          reportService.getReports({ limit: 3 })
        ]);
        ...
        if (reportsResponse.success && reportsResponse.data) {
          ...
          const transformedReports = reportsResponse.data.slice(0, 3).map(report => ({
            ...
            title: report.title,
            type: report.type,
            status: report.status === 'published' ? 'completed' : report.status,
            ...
            <div className="flex items-center">
              ...
              <div className="ml-5 w-0 flex-1">
                <dl>
                  ...
                  <dd className="text-3xl font-bold text-gray-900">
                    {metrics?.activeScheduledReports || 0}
                  </dd>
                </dl>
              </div>
            </div>
